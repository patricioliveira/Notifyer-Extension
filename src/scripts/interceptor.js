(function() {
    class Order {
        ...
        convertToWhatsAppFormat(originalMessage) {
            // Substitui os códigos de escape por seus caracteres correspondentes
            var decodedMessage = decodeURIComponent(originalMessage);

            // Substitui quebras de linha por %0A
            var whatsappFormattedMessage = decodedMessage.replace(/\n/g, '%0A');

            return whatsappFormattedMessage;
        }

        sendToAPI(apiUrl) {
            // Verifica se a URL da API foi fornecida
            if (!apiUrl) {
                console.error('URL da API não fornecida.');
                return;
            }

            // Constrói o objeto de dados para enviar à API de mensagens
            var messageData = {
                phone: "55"+this.client_phone,  // Substitua pelo campo correto da instância Order
                isGroup: false,  // Substitua conforme necessário
                message: this.convertToWhatsAppFormat(this.messages.wpp_message),  // Substitua pela mensagem desejada
            };
            ...
            this.addEventListener('load', function () {
                try {
                    if (this.responseText.trim() !== '') {
                        ...
                        // Cria uma instância da classe Order
                        var orderObject = new Order(responseObject);
                        console.log('Corpo da resposta:', orderObject);

                        // Envia os dados para a API de mensagens (substitua a URL da API real)
                        orderObject.sendToAPI('http://localhost:21465/api/XABLAU2:$2b$10$ztkcqNYbpGkNbHjsyYcx3.JrhuIwa8xOaW_3gRoc5eDKyUgiaOwHa/send-message');
                    } else {
